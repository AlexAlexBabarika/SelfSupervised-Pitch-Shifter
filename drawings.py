...
def waveform_chart(
    ...
    rms_top = base.mark_line(color="#ec4899", strokeWidth=1.2).encode(
        x="time:Q", y="rms:Q"
    )
    rms_bot = (
        base.transform_calculate(neg="-datum.rms")
        .mark_line(color="#ec4899", strokeWidth=1.2)
        .encode(
            x="time:Q",
            y=alt.Y("neg:Q"),
        )
...
def comparison_chart(
    stages: list[tuple[np.ndarray, int | float, str]], n_buckets: int = 1500
) -> alt.Chart:
    frames = []
    for audio, sr, label in stages:
        ...
        frames.append(
            pl.DataFrame(
                {
                    "time": (np.arange(buckets.shape[0]) * bucket_size / sr).astype(
    ...
    return (
        ...
        .encode(
            ...
            y2="max:Q",
            color=alt.Color(
                "stage:N", legend=alt.Legend(title=None), scale=alt.Scale(scheme="set2")
            ),
            row=alt.Row(
                "stage:N",
                title=None,
        ...
        .properties(width=760, height=140)
        .configure_view(strokeOpacity=0)
        .configure_axis(grid=False)
    )
...
def f0_chart(
    ...
    t, freq, conf = t[mask], freq[mask], conf[mask]
    if freq.size == 0:
        raise ValueError(
            "No voiced frames above confidence_threshold; lower it or check the input."
        )

    lo, hi = float(freq.min()), float(freq.max())
    pad = (hi / lo) ** 0.05 if log_scale and lo > 0 else (hi - lo) * 0.05 or 1.0
    domain = [lo / pad, hi * pad] if log_scale else [lo - pad, hi + pad]

    df = pl.DataFrame({"time": t, "f0": freq, "confidence": conf})
    y_scale = (
        alt.Scale(type="log", domain=domain)
        if log_scale
        else alt.Scale(domain=domain, zero=False)
    )
    return (
        alt.Chart(df)
        .mark_circle(size=18, color=color)
        .encode(
            x=alt.X("time:Q", title="Time (s)"),
            y=alt.Y("f0:Q", title="Frequency (Hz)", scale=y_scale),
            opacity=alt.Opacity(
                "confidence:Q",
